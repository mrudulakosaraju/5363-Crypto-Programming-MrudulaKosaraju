...
class PlayFair:
	...
	# it decodes the cipher text
		...
		for row in self.Square:
			if digraph[0] in row and digraph[1] in row:
				newDigraph[0] = row[ 4 if row.index(digraph[0])-1 < 0 else row.index(digraph[0])-1]
				newDigraph[1] = row[4 if row.index(digraph[1])-1 < 0 else row.index(digraph[1])-1]
				return newDigraph 
...
#It cleans the input message to alpha numeric characters
#It decodes and encrypts the message
...
if(option == "1"):
	...
	#if the message length is odd append 'X' to message
	if len(myCipher.Message)%2!=0:
		myCipher.Message = myCipher.Message + 'X'
	while i < (len(myCipher.Message)-1):
		A = myCipher.getCodedDigraph([myCipher.Message[i],myCipher.Message[i+1]])
		B =  B + A[0] + A[1]
		i = i + 2
	print(A)
elif(option == "2"):
	...
	while i < (len(myCipher.Message)-1):
		A1 = myCipher.Decode([myCipher.Message[i],myCipher.Message[i+1]])
		B = B + A1[0] + A1[1]
		i = i + 2
	print(B)
elif(option == "3"):
